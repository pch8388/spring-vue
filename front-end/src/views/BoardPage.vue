...
import AddMemberModel from '@/modals/AddMemberModal.vue';
...
export default {
  ...
  methods: {
    ...
    dismissActiveForms(event) {
      ...
      if (target.closest('.add-card-form') || target.closest('.add-card-button')) {
        dismissAddCardForm = false;
      }
      if (target.closest('.add-list-form') || target.closest('.add-list-button')) {
    ...
    addCardList() {
      if (!this.addListForm.name) return;
      ...
      cardListService.add(cardList).then(saveCardList => {
        this.cardLists.push({
          id: saveCardList.id,
          name: saveCardList.name,
          cards: [],
          cardForm: {
            open: false,
            title: '',
    ...
    onCardDragEnded(event) {
      const fromListId = event.form.dataset.listId;
    ...
    subscribeToRealTimeUpdate() {
      this.$rt.subscribe(`/board/${this.board.id}`, this.onRealTimeUpdate);
    },
    unsubscribeFromRealTimeUpdate() {
      this.$rt.unsubscribe(`/board/${this.board.id}`, this.onRealTimeUpdate);
    },
